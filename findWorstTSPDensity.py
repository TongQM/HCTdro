...
while (abs(UB - LB) > epsilon):
    ...
    UB, UB_error = integrate.dblquad(upper_integrand, 0, 2*np.pi, lambda _: 0, lambda _: region.radius, args=(demands_locations, lambdas_bar, v_bar),epsabs=1e-3)
    time2 = time.time()
    print(f'Find upper bound: Upper bound is {UB}, with error {UB_error}, took {time2 - time1}s.')
    ...
    LB, LB_error = integrate.dblquad(lower_integrand, 0, 2*np.pi, lambda _: 0, lambda _: region.radius, args=(demands_locations, lambdas_bar, v_tilde), epsabs=1e-3)
    time3 = time.time()
    print(f'Find lower bound: Lower bound is {LB}, with error {LB_error}, took {time3 - time2}s.')
    ...
    for i in range(len(demands)):
        ...
        g[i], g_error = integrate.dblquad(integrandi, 0, 2*np.pi, lambda _: 0, lambda _: region.radius, args=(demands_locations, lambdas_bar, v_bar))
