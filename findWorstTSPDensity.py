...
def f_bar(r, theta, demands_locations, lambdas_bar, v_bar):
    ...
    return pow(1/4 * (v_bar[0]*np.min(np.array([norm_func(x_cdnt, demands_locations[i]) - lambdas_bar[i] for i in range(demands_locations.shape[0])])) + v_bar[1]), -2)

@nb.njit
def f_tilde(r, theta, demands_locations, lambdas_bar, v_tilde):
    x_cdnt = np.array([r*np.cos(theta), r*np.sin(theta)])
    return 1/4 * np.sum(np.array([region_indicator(i, x_cdnt, lambdas_bar, demands_locations) * pow((v_tilde[0]*norm_func(x_cdnt, demands_locations[i]) + v_tilde[i+1]), -2) for i in range(demands_locations.shape[0])]))
...
# f = findWorstTSPDensity(region, demands, t=1, epsilon=1)
t, epsilon = 1, 1
n = demands.size
UB, LB = np.inf, -np.inf
perturbation = 1e-4
lambdas_bar = np.array([0, 0]) + perturbation
polyhedron = Polyhedron(np.eye(n), region.diam*np.ones(n), np.ones((1, n)), 0, n)
k = 1
while (UB - LB > epsilon):
    print(f'Iteration {k} begins: \n')
    ...
    upper_integrand = lambda r, theta, demands_locations, lambdas_bar, v_bar: r*np.sqrt(f_bar(r, theta, demands_locations, lambdas_bar, v_bar))
    UB, UB_error = integrate.dblquad(upper_integrand, 0.0001, 2*np.pi, lambda _: 0.0001, lambda _: region.radius, args=(demands_locations, lambdas_bar, v_bar),epsabs=1e-3)
    time2 = time.time()
    print(f'Find upper bound: Upper bound is {UB}, with error {UB_error}, took {time2 - time1}s.')
    ...
    LB, LB_error = integrate.dblquad(lower_integrand, 0.0001, 2*np.pi, lambda _: 0, lambda _: region.radius, args=(demands_locations, lambdas_bar, v_tilde), epsabs=1e-3)
    time3 = time.time()
    print(f'Find lower bound: Lower bound is {LB}, with error {LB_error}, took {time3 - time2}s.')
    ...
    for i in range(len(demands)):
        ...
        g[i], g_error = integrate.dblquad(integrandi, 0.0001, 2*np.pi, lambda _: 0, lambda _: region.radius, args=(demands_locations, lambdas_bar, v_bar))
